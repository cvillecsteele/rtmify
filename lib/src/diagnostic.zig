...
pub const all_codes = [_]Code{
    ...
    E.path_traversal,
    E.xlsm_macro,
    E.missing_content_types,
    E.missing_workbook,
    E.missing_workbook_rels,
    E.missing_shared_strings,
    E.missing_worksheet,
    E.requirements_tab_missing,
    E.ambiguous_tab,
    E.tab_synonym_match,
    E.tab_substring_match,
    E.tab_fuzzy_match,
    E.optional_tab_missing,
    E.column_synonym_match,
    E.column_ambiguous,
    E.id_column_guessed,
    E.id_column_missing,
    E.id_paren_stripped,
    E.id_hyphen_stripped,
    E.row_no_id,
    E.duplicate_id,
    E.duplicate_test_id,
    E.numeric_text_mapped,
    ...
    E.req_compound,
    E.req_vague,
    E.req_obsolete_traced,
    E.risk_score_mismatch,
    E.risk_unmitigated,
    E.risk_residual_no_init,
    E.risk_residual_exceeds,
    E.test_group_empty,
    E.ref_not_found,
    E.ref_wrong_type,
    E.repo_path_not_exist,
    E.repo_not_a_directory,
    E.repo_no_git_dir,
    E.repo_not_readable,
    E.repo_git_not_found,
    E.repo_git_too_old,
    E.git_log_failed,
    E.git_blame_failed,
    E.git_commit_parse_err,
    E.git_blame_parse_err,
    E.git_timeout,
    E.annotation_unknown_req,
    E.annotation_not_comment,
    E.annotation_file_large,
    E.annotation_binary,
    E.annotation_unknown_ext,
    E.annotation_multi_line,
    E.chain_incomplete,
    E.di_without_do,
    E.hlr_without_llr,
    E.asil_not_specified,
    E.asil_inheritance,
    E.chain_gap,
...
pub const catalog = [_]CatalogEntry{
    .{ .code = 101, .title = "File not found", .category = .file_io },
    .{ .code = 102, .title = "Path is a directory, not a file", .category = .file_io },
    .{ .code = 103, .title = "File cannot be read", .category = .file_io },
    .{ .code = 104, .title = "File is empty", .category = .file_io },
    .{ .code = 105, .title = "File too large (> 500 MB)", .category = .file_io },
    .{ .code = 106, .title = "Unexpected file extension", .category = .file_io },
    .{ .code = 201, .title = "OLE2 / legacy .xls format — re-save as .xlsx", .category = .workbook_container },
    .{ .code = 202, .title = "OpenDocument Spreadsheet (.ods) — re-save as .xlsx", .category = .workbook_container },
    .{ .code = 203, .title = "HTML document saved with .xlsx extension", .category = .workbook_container },
    .{ .code = 204, .title = "Not a ZIP archive", .category = .workbook_container },
    .{ .code = 205, .title = "Password-protected workbook", .category = .workbook_container },
    .{ .code = 206, .title = "ZIP bomb detected (extreme compression ratio)", .category = .workbook_container },
    .{ .code = 207, .title = "Binary Excel format (.xlsb) — re-save as .xlsx", .category = .workbook_container },
    .{ .code = 208, .title = "ZIP entry with path traversal sequence skipped", .category = .workbook_container },
    .{ .code = 209, .title = "Macro-enabled workbook (.xlsm) — macros ignored", .category = .workbook_container },
    .{ .code = 301, .title = "Missing [Content_Types].xml — file may be corrupt", .category = .workbook_structure },
    .{ .code = 302, .title = "Missing xl/workbook.xml", .category = .workbook_structure },
    .{ .code = 303, .title = "Missing xl/_rels/workbook.xml.rels", .category = .workbook_structure },
    .{ .code = 304, .title = "xl/sharedStrings.xml absent — treating all cells as inline values", .category = .workbook_structure },
    .{ .code = 305, .title = "Worksheet file listed in workbook but missing from ZIP", .category = .workbook_structure },
    .{ .code = 401, .title = "No matching 'Requirements' tab found", .category = .tab_discovery },
    .{ .code = 402, .title = "Multiple tabs match the same name — using first", .category = .tab_discovery },
    .{ .code = 403, .title = "Tab identified by known synonym", .category = .tab_discovery },
    .{ .code = 404, .title = "Tab identified by substring match", .category = .tab_discovery },
    .{ .code = 405, .title = "Tab identified by fuzzy (Levenshtein) match", .category = .tab_discovery },
    .{ .code = 406, .title = "Optional tab not found — related data will be absent", .category = .tab_discovery },
    .{ .code = 501, .title = "Column header matched by known synonym", .category = .column_mapping },
    .{ .code = 502, .title = "Multiple columns match the same field — using leftmost", .category = .column_mapping },
    .{ .code = 503, .title = "ID column not found by name; inferred from data pattern", .category = .column_mapping },
    .{ .code = 504, .title = "ID column not found — rows in this tab will be skipped", .category = .column_mapping },
    .{ .code = 601, .title = "Parenthetical suffix removed from ID", .category = .row_parsing },
    .{ .code = 602, .title = "Leading or trailing hyphens removed from ID", .category = .row_parsing },
    .{ .code = 603, .title = "Row has content but no ID — skipped", .category = .row_parsing },
    .{ .code = 604, .title = "Duplicate ID in this tab — subsequent row skipped", .category = .row_parsing },
    .{ .code = 605, .title = "Duplicate test case ID — subsequent row skipped", .category = .row_parsing },
    ...
    .{ .code = 704, .title = "Compound requirement: multiple 'shall' clauses", .category = .semantic_validation },
    .{ .code = 705, .title = "Vague or ambiguous term in requirement statement", .category = .semantic_validation },
    .{ .code = 706, .title = "Obsolete requirement still has active trace links", .category = .semantic_validation },
    .{ .code = 707, .title = "Risk severity present without likelihood (or vice versa)", .category = .semantic_validation },
    .{ .code = 708, .title = "High-risk score (> 12) with no mitigation", .category = .semantic_validation },
    .{ .code = 709, .title = "Residual risk scores present but initial scores absent", .category = .semantic_validation },
    .{ .code = 710, .title = "Residual risk score exceeds initial risk score", .category = .semantic_validation },
    .{ .code = 711, .title = "Test group has no associated test cases", .category = .semantic_validation },
    .{ .code = 801, .title = "Cross-reference target not found in the graph", .category = .cross_reference },
    .{ .code = 802, .title = "Cross-reference target has an unexpected node type", .category = .cross_reference },
    .{ .code = 901, .title = "Repository path does not exist", .category = .repo_configuration },
    .{ .code = 902, .title = "Repository path is not a directory", .category = .repo_configuration },
    .{ .code = 903, .title = "Repository path has no .git directory", .category = .repo_configuration },
    .{ .code = 904, .title = "Repository path is not readable", .category = .repo_configuration },
    .{ .code = 905, .title = "git executable not found in PATH", .category = .repo_configuration },
    .{ .code = 906, .title = "git version is too old (requires 2.0+)", .category = .repo_configuration },
    .{ .code = 1001, .title = "git log command failed", .category = .git_integration },
    .{ .code = 1002, .title = "git blame command failed", .category = .git_integration },
    .{ .code = 1003, .title = "Failed to parse git commit output", .category = .git_integration },
    .{ .code = 1004, .title = "Failed to parse git blame output", .category = .git_integration },
    .{ .code = 1005, .title = "git command timed out (> 10s)", .category = .git_integration },
    .{ .code = 1101, .title = "Annotation references unknown requirement ID", .category = .annotation_scanning },
    .{ .code = 1102, .title = "Annotation found outside of a comment block", .category = .annotation_scanning },
    .{ .code = 1103, .title = "Source file too large to scan (> 10 MB)", .category = .annotation_scanning },
    .{ .code = 1104, .title = "Source file appears to be binary — skipped", .category = .annotation_scanning },
    .{ .code = 1105, .title = "Source file has unknown extension — annotation scanning skipped", .category = .annotation_scanning },
    .{ .code = 1106, .title = "Multi-line annotation detected — only first line captured", .category = .annotation_scanning },
    .{ .code = 1201, .title = "Traceability chain is incomplete for this node", .category = .profile_traceability },
    .{ .code = 1202, .title = "Design Input has no linked Design Output", .category = .profile_traceability },
    .{ .code = 1203, .title = "High-level requirement has no linked low-level requirement", .category = .profile_traceability },
    .{ .code = 1204, .title = "ASIL level not specified for this requirement", .category = .profile_traceability },
...
pub fn categoryFor(code: Code) ?Category {
    for (catalog) |e| {
        if (e.code == code) return e.category;
    }
    return null;
}
...
test "all_codes have categories" {
    for (all_codes) |code| {
        try testing.expect(categoryFor(code) != null);
    }
}
...
test "lookupTitle returns known title" {
    ...
    try testing.expectEqual(Category.repo_configuration, categoryFor(901).?);
}
