...
pub fn ingest(ctx: *const internal.IngestContext, sheet: internal.SheetData, stats: *internal.IngestStats) !void {
    ...
    const c_id = columns.resolveCol(headers, data, "ID", columns.di_id_syns, sheet.name, diag, true);
    const c_desc = columns.resolveCol(headers, &.{}, "Description", columns.di_desc_syns, sheet.name, diag, false);
    const c_src = columns.resolveCol(headers, &.{}, "Source Requirement", columns.di_src_req_syns, sheet.name, diag, false);
    const c_status = columns.resolveCol(headers, &.{}, "Status", columns.di_status_syns, sheet.name, diag, false);
    ...
    for (data, 0..) |row, ri| {
        ...
        const src_raw = internal.cell(row, c_src);
        if (!normalize.isBlankEquivalent(src_raw)) {
            for (try normalize.splitIds(src_raw, a)) |part| {
