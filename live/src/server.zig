...
fn handleRequest(req: *std.http.Server.Request, ctx: ServerCtx) !void {
    ...
    const access = routeAccess(req.head.method, path);
    ...
    if (req.head.method == .GET) {
        if (std.mem.eql(u8, path, "/nodes")) {
            ...
            const artifact_id = try decodePathParam(path["/api/v1/design-artifacts/".len..], alloc);
        ...
        if (std.mem.eql(u8, path, "/mcp")) {
            ...
            const artifact_id = try decodePathParam(path["/api/v1/design-artifacts/".len .. path.len - "/reingest".len], alloc);
